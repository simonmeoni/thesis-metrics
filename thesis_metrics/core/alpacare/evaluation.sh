...
for model in "${refer_model_name[@]}"; do
    data_path="data/alpacare/eval/reference_outputs/${model}/iCliniq_output.jsonl"
    batch_dir=$(dirname "$2")
    max_test_number=1000

    python thesis_metrics/core/alpacare/gpt_eval.py \
        --model_output "$2" \
    ...
    python thesis_metrics/core/alpacare/preference_eval.py \
        --compared_model "$1" \
